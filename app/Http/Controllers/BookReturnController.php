...
class BookReturnController extends Controller
{
    public function index(Request $request){
        ...
        $items = BookReturn::orderBy('id','DESC')->paginate(5);


        return view('book_return.index',compact('items'))->with('i', ($request->input('page', 1) - 1) * 5);
    }


    public function create(){
       /* $cities = DB::table("book_issue")
            ->where("id",7)
            ->get();
        echo "<pre>";
        print_r($cities);
    ...
    public function store(Request $request){
       ...
       /* $bookReturn = new BookReturn();
        $bookReturn->book_issue_code = $data['book_issue_code'];
        $bookReturn->user_id = $data['user_id'];
        $bookReturn->admin_id = $admin_id;
        $bookReturn->type = $data['type'];
        $bookReturn->return_date = $data['return_date'];
        ...
        if($return_date>$end_issue_date){
            $bookReturn->late_count = $return_date-$end_issue_date;
        }
    ...
    public function show($id){
        ...
        return view('book_return.show',$data);
    }

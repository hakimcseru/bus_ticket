...
class PublicSslCommerzPaymentController extends Controller
{
    ...
    public function index(Request $request) 
    {
            ...
            //$request->all();
            $name0=$request->input('name0');
            $email=$request->input('email');
            $contact_number=$request->input('contact_number');
            $order_id_final=$request->input('order_id_final');
            $total_amount_payable_hidden=$request->input('total_amount_payable_hidden');
           ...
           // exit();
            ...
            $post_data = array();
            $post_data['total_amount'] = $total_amount_payable_hidden; # You cant not pay less than 10
            $post_data['currency'] = "BDT";
            $post_data['tran_id'] = $order_id_final; // tran_id must be unique
            ...
            $post_data['cus_name'] = $name0;
            $post_data['cus_email'] = $email;
            $post_data['cus_add1'] = 'Dhaka Mohakhali';
            $post_data['cus_add2'] = "";
            $post_data['cus_city'] = "";
            $post_data['cus_state'] = "";
            ...
            $update_product = DB::table('booking')
    ...
    public function success(Request $request) 
    {
       ...
       // exit();
        ...
        $order_detials = DB::table('booking')
                            ->where('id', $tran_id)
                            ->select('id', 'order_status','currency','price')->first();
     ...
     public function ipn(Request $request) 
    {
      ...
      if($request->input('tran_id')) #Check transation id is posted or not.
      {
        ...
        #Check order status in order tabel against the transaction id or order id.
         $order_details = DB::table('booking')
                            ->where('id', $tran_id)
                            ->select('id', 'order_status','currency','price')->first();
